...
int SHPAPI_CALL1(*) SHPSearchDiskTree(SAFile *file, double *padfBoundsMin, double *padfBoundsMax, int *pnShapeCount)
{
    ...
    // SAFile'e uygun hook'lar atanıyor
    sDiskTree.sHooks.FSeek = SADFSeek;
    sDiskTree.sHooks.FRead = SADFRead;

    sDiskTree.fpQIX = file;
...
static void SHPWriteTreeNode(SAFile *file, SHPTreeNode *node, const SAHooks *psHooks)
{
    ...
    unsigned char *pabyRec = (unsigned char *)malloc(sizeof(double) * 4 + (3 * sizeof(int)) + (node->nShapeCount * sizeof(int)));
    ...
    // Minx, Miny, Maxx, Maxy
    memcpy(pabyRec + 4, node->adfBoundsMin, sizeof(double) * 2);
    memcpy(pabyRec + 20, node->adfBoundsMax, sizeof(double) * 2);

    memcpy(pabyRec + 36, &node->nShapeCount, 4);
    j = node->nShapeCount * sizeof(int);
    if (j > 0)
        memcpy(pabyRec + 40, node->panShapeIds, j);
    memcpy(pabyRec + 40 + j, &node->nSubNodes, 4);

    psHooks->FWrite(pabyRec, 44 + j, 1, file);
    free(pabyRec);

    for (i = 0; i < node->nSubNodes; i++)
    {
        if (node->apsSubNode[i] != SHPLIB_NULLPTR)
        {
            SHPWriteTreeNode(file, node->apsSubNode[i], psHooks);
        }
    }
}
...
int SHPWriteTreeLL(SHPTree *tree, const char *filename, const SAHooks *psHooks)
{
    ...
    abyBuf[4] = 1;                         // Versiyon
    abyBuf[5] = abyBuf[6] = abyBuf[7] = 0; // Rezerv

    psHooks->FWrite(abyBuf, 8, 1, file);
